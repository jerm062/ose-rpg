...
let textMode = false;
let textLetter = { text: '', color: 'rgb(255,0,0)', font: '12px "Tiny5", monospace', blink: true };
let blinkOn = true;
...
function createDungeonMap() {
  ...
  mapLetters = Array.from({ length: height }, () => Array(width).fill(null));
  const rooms = [];
  const roomCount = 8;
  ...
  for (let y = 0; y < mapData.length; y++) {
    for (let x = 0; x < mapData[y].length; x++) {
      ...
      if (mapLetters[y] && mapLetters[y][x]) {
        const ltr = mapLetters[y][x];
        const color = ltr.color || '#f00';
        const font = ltr.font || '12px "Tiny5", monospace';
        const blink = ltr.blink !== false;
        const ch = ltr.text || ltr;
        ctx.fillStyle = color;
        ctx.font = font;
        if (!blink || blinkOn) {
          ctx.fillText(
...
socket.on('mapData', (data) => {
  ...
  mapLetters = data.letters || mapData.map(r => r.map(() => null));
  for (let y = 0; y < mapLetters.length; y++) {
    for (let x = 0; x < mapLetters[y].length; x++) {
      const l = mapLetters[y][x];
      if (typeof l === 'string') {
        mapLetters[y][x] = { text: l, color: 'rgb(255,0,0)', font: '12px "Tiny5", monospace', blink: true };
      }
    }
  }
...
canvas.addEventListener('click', (ev) => {
  ...
  if (mapData[y] && typeof mapData[y][x] !== 'undefined') {
    if (textMode) {
      const letter = { ...textLetter };
      mapLetters[y][x] = letter;
      socket.emit('setMapLetter', { x, y, letter });
      drawMap();
      return;
    }
    if (ev.ctrlKey) {
...
if (menuTextBtn) menuTextBtn.onclick = () => {
  if (textMode) {
    ...
    const l = prompt('Letter to paint:', textLetter.text || 'A');
    if (!l) return;
    const rgb = prompt('RGB color (e.g. 255,0,0):',
      textLetter.color.replace(/rgb\((.*)\)/, '$1'));
    const font = prompt('Font (CSS format):', textLetter.font);
    const blinkResp = prompt('Blink? (y/n):', textLetter.blink ? 'y' : 'n');
    textLetter = {
      text: l[0],
      color: rgb ? `rgb(${rgb})` : textLetter.color,
      font: font || textLetter.font,
      blink: blinkResp ? blinkResp.toLowerCase().startsWith('y') : textLetter.blink
    };
    textMode = true;
